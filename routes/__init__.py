...
from . import chrome, eagle
from .utils import register_routes as utils_register_routes
...
def _path_is_within_roots(target_path, roots):
    ...
    try:
        ...
        for root in roots:
            ...
            if (
                os.path.commonpath([normalized_target, normalized_root])
                == normalized_root
            ):
                return True
    except ValueError:
...
def register_routes_debug(app):
    ...
    def debug_print():
        ...
        return render_template(
            "test_arg.html", title="All in One", df_to_post=df_to_post
        )
...
def register_routes(app):
    ...
    def index():
        ...
        try:
            ...
            image_only = [
                item for item in image_payload if item.get("media_type") == "image"
            ]
            video_only = [
                item for item in image_payload if item.get("media_type") == "video"
            ]
            ...
            eagle_tags = (
                random.sample(tag_data, min(20, len(tag_data))) if tag_data else []
            )
            ...
            if folder_data:
                ...
                for key, items in clusters_map.items():
                    ...
                    curated_clusters.append(
                        {"title": f"{key} 精選合集", "items": items[:5]}
                    )
    ...
    def open_filesystem_path():
        ...
        target_directory = (
            ...
            else os.path.dirname(decoded_path)
        )
